...
/*   Updated: 2025/02/20 14:19:26 by meferraz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_status	ft_set_up_signals(void)
{
	...
	if (sigaction(SIGINT, &sa_int, NULL) == -1)
		return (ft_printf(STDERR_FILENO, ERR_SIGNAL_SETUP_INT), ERROR);
	sa_quit.sa_handler = ft_handle_sigquit;
	sigemptyset(&sa_quit.sa_mask);
	sa_quit.sa_flags = 0;
	if (sigaction(SIGQUIT, &sa_quit, NULL) == -1)
		return (ft_printf(STDERR_FILENO, ERR_SIGNAL_SETUP_QUIT), ERROR);
	return (SUCCESS);
}
